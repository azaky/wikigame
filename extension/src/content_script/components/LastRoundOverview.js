  ...
  const onShowDetails = (player) => {
    ...
    toast(
      () => (
        <div>
          <div>
            <h3>{player.username}'s last round</h3>
          </div>
          <div>
          ...
          {player.finished ? (
            <div>
              Score = {player.score}, Time Taken = {player.timeTaken}{' '}
              {player.timeTaken > 1 ? ' seconds' : ' second'}
            </div>
          ) : null}
          <div>
            {player.path.map((p) => (
    ...
    <nav class="vector-menu vector-menu-portal portal">
      ...
      <div class="body vector-menu-content">
        <ul>
          <li>
            <b>
              <a href={util.getLink(round.start)}>
                {round.start.replace(/_/g, ' ')}
              </a>
              {' to '}
              <a href={util.getLink(round.target)}>
                {round.target.replace(/_/g, ' ')}
              </a>
            </b>
          </li>
        </ul>
      ...
      <div class="body vector-menu-content">
        <ul>
          ...
          {round.result.map((player) => {
            return (
              <li>
                {player.username} {!player.finished ? '(not finished) ' : ''}
                <a onClick={() => onShowDetails(player)}>
                  (
                  {metrics === 'click' || !player.finished
                    ? `${player.clicks} ${
                        player.clicks > 1 ? 'clicks' : 'click'
                      }`
                    : metrics === 'time'
                    ? `${player.timeTaken} ${
                        player.timeTaken > 1 ? 'seconds' : 'second'
